...
function loadProducts() {
    ...
    $.getJSON(jsonURL, function (json) {
        ...
        $.each(json.products, function () {
            ...
            //   <a href=\'#' + this.details + '\' > </a>' + this.name + '</h3></li>';
            ...
            imgList += '<li>\
                ...
                + this.name + '</a>\
                    <h3 class='+ this.name + ' > SGD ' + this.price + '</h3></h3><h3>\
                    </h3></li>';
        });
        imgList += '</ul>'
...
function populateCurrency() {
    ...
    var selectionBox = document.getElementById("currency");
    // Read the currency json file, which contains the currency code and exchange rate 
    $.getJSON(jsonURL, function (json) {
    ...
    if (hash == "#products") {
        ...
        $.getJSON(jsonURL, function (json) {
            $.each(json.products, function () {
                ...
                $('.' + this.name).html(currencyCode + ' ' + newPrice);
